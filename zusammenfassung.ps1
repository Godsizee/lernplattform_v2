...
# Backslashes (\) und Punkte (.) sind für Regex mit einem Backslash maskiert.
$ExcludeFilePattern = "public\\assets\\js\\vendor\\marked\.esm\.js$|public\\assets\\js\\vendor\\purify\.es\.js$"
...
# Alle Dateipfade sammeln, ABER die ausgeschlossenen Ordner UND Dateien ignorieren.
$allFilePaths = Get-ChildItem -Path $SourceDirectory -Recurse -File | 
    Where-Object { 
        ($_.FullName -notmatch "(?i)\\($($ExcludeDirectoryPattern))\\") -and 
        ($_.FullName -notmatch "(?i)($ExcludeFilePattern)") 
    } | 
...
Get-ChildItem -Path $SourceDirectory -Recurse -Include $FileTypesToProcess -File | ForEach-Object {
    ...
    # Überprüft, ob der Pfad ein ausgeschlossener Ordner ODER eine ausgeschlossene Datei ist
    If (($filePath -match "(?i)\\($($ExcludeDirectoryPattern))\\") -or ($filePath -match "(?i)($ExcludeFilePattern)")) {
        Write-Host "Überspringe (Ausgeschlossen): $($filePath)" -ForegroundColor DarkYellow
        Return
    }
